	     ...
	     and defined $extracted_yml->{yml})
    {
		...
		if (defined $parsed_yml)
		{
		    ...
		    foreach my $fn (keys %{$parsed_yml})
		    {
			 ...
			 if (exists $pagestate{$page}{ymlfront}{tag}
				     and $pagestate{$page}{ymlfront}{tag})
			 {
			   if(! ref $pagestate{$page}{ymlfront}{tag})
			   {
			     my %tag_param;
				  $tag_param{page} = $page;
				  my @tags = split / /, $pagestate{$page}{ymlfront}{tag};
				  foreach my $tag (@tags) {
			  	    $tag_param{$tag} = $tag;
			     }
			     # print "-- Agregando '".$pagestate{$page}{ymlfront}{tag}."'\n";
