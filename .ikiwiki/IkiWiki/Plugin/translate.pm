...
package IkiWiki::Plugin::translate;
...
my @origneedsbuild;
my %origsubs;
my @slavelanguages; # language codes ordered as in config translate_slave_languages
...
sub import {
	hook(type => "getsetup", id => "translate", call => \&getsetup);
	hook(type => "checkconfig", id => "translate", call => \&checkconfig,
		last => 1);
	hook(type => "needsbuild", id => "translate", call => \&needsbuild);
	hook(type => "scan", id => "translate", call => \&scan, last => 1);
	hook(type => "filter", id => "translate", call => \&filter);
	hook(type => "htmlize", id => "translate", call => \&htmlize);
	hook(type => "pagetemplate", id => "translate", call => \&pagetemplate, last => 1);
	hook(type => "rename", id => "translate", call => \&renamepages, first => 1);
	hook(type => "delete", id => "translate", call => \&mydelete);
	hook(type => "rendered", id => "translate", call => \&rendered);
	hook(type => "checkcontent", id => "translate", call => \&checkcontent);
	hook(type => "canremove", id => "translate", call => \&canremove);
	hook(type => "canrename", id => "translate", call => \&canrename);
	hook(type => "editcontent", id => "translate", call => \&editcontent);
	hook(type => "formbuilder_setup", id => "translate", call => \&formbuilder_setup, last => 1);
	hook(type => "formbuilder", id => "translate", call => \&formbuilder);
...
sub checkconfig () {
	if (exists $config{translate_master_language}) {
		if (! ref $config{translate_master_language}) {
			($master_language_code, $master_language_name)=
				splitlangpair($config{translate_master_language});
		}
		else {
			$master_language_code=$config{translate_master_language}{code};
			$master_language_name=$config{translate_master_language}{name};
			$config{translate_master_language}=joinlangpair($master_language_code, $master_language_name);
		}
	}
	...
	if (ref $config{translate_slave_languages} eq 'ARRAY') {
		foreach my $pair (@{$config{translate_slave_languages}}) {
	...
	elsif (ref $config{translate_slave_languages} eq 'HASH') {
		%slavelanguages=%{$config{translate_slave_languages}};
		@slavelanguages = sort {
			$config{translate_slave_languages}->{$a} cmp $config{translate_slave_languages}->{$b};
		} keys %slavelanguages;
		$config{translate_slave_languages}=[
	...
	if (! exists $config{translate_translatable_pages} ||
	    ! defined $config{translate_translatable_pages}) {
		$config{translate_translatable_pages}="";
	}
	if (! exists $config{translate_link_to} ||
	    ! defined $config{translate_link_to}) {
		$config{translate_link_to}='default';
	}
	elsif ($config{translate_link_to} !~ /^(default|current|negotiated)$/) {
		warn(sprintf(gettext('%s is not a valid value for translate_link_to, falling back to translate_link_to=default'),
			     $config{translate_link_to}));
		$config{translate_link_to}='default';
	}
	elsif ($config{translate_link_to} eq "negotiated" && ! $config{usedirs}) {
		warn(gettext('translate_link_to=negotiated requires usedirs to be enabled, falling back to translate_link_to=default'));
		$config{translate_link_to}='default';
	}

	# hack ya no usamos pot files
...
sub pagetemplate (@) {
	...
	# hack: esta variable ayuda a traducir en los .tmpl <TMPL_IF LANG_EN>..</TMPL_IF>
	if ($template->query(name => "lang_".$lang_code)) {
		$template->param("lang_".$lang_code => $lang_code);
	}
	...
	if ($template->query(name => "pageid")) {
	   my $pageid = $page;
	   $pageid =~ tr/\//_/;
	   $pageid =~ s/[[:^ascii:]]/_/g;
		$template->param(pageid => $pageid);
	}
...
sub renamepages (@) {
	...
	while (my ($lang, $otherpage) = each %otherpages) {
		push @ret, {
			...
			destfile => $torename{dest}.".".$lang.".translate",
			required => 0,
		};
	}
	return @ret;
...
#		my $updated_pot_file=0;
		...
		# hack solo checquear si existen la .tranlate files sino crearlas
		my @pofiles;
...
#		}
	...
	if ($updated_po_files) {
		commit_and_refresh(
			gettext("updated TRANSLATE files"));
	}
}
...
# Implement translate_link_to 'current' and 'negotiated' settings.
sub mybestlink ($$) {
	my $page=shift;
	my $link=shift;

	return $origsubs{'bestlink'}->($page, $link)
		if defined $config{translate_link_to} && $config{translate_link_to} eq "default";
...
sub myurlto ($;$$) {
	...
	# avoid, i.e. when translate_link_to = negotiated.
	...
	if ($config{translate_link_to} eq "negotiated") {
		my @caller = caller(1);
		my $use_orig = 0;
...
sub istranslatablefile ($) {
	...
	return 0 if ! defined $type || $type eq 'translate';
	# hack ya no usamos pot files!
#	return 0 if $file =~ /\.pot$/;
   # hack revisar contenido

  if ($type eq 'mdwn' && exists $pagesources{pagename($file)}) {
	  my $pathfile = srcfile($pagesources{pagename($file)});
	  (my $name, my $dir, my $suffix) = fileparse($pathfile, qr/\.[^.]*/);
	  $dir='' if $dir eq './';
	  foreach my $ll (@slavelanguages) {
	    if (-e $dir.'/'.$name.'.'.$ll.'.translate'){
	      #print '-- translatable: '.$dir.$name.'.'.$ll.'.translate'."\n";
	      return 1;
	    }
	  }
  }

#  if (exists $pagesources{pagename($file)} ){
#		my $content=readfile(srcfile($pagesources{pagename($file)}));
#		if ($content =~ /translatable=yes/){
#			# print "translatable=yes --> $file \n";
#			return 1;
#		}
#	}

	
	return 0 if ! defined $config{translate_translatable_pages};
	return 1 if pagespec_match(pagename($file), $config{translate_translatable_pages});
	return;
}
...
# then the codes are ordered the same way as in translate_slave_languages, if it is
...
#sub potfile ($) {
# hack ya nadie usa esta funcion
#	my $masterfile=shift;
#
...
sub pofile ($$) {
	...
	(my $name, my $dir, my $suffix) = fileparse($masterfile, qr/\.[^.]*/);
	$dir='' if $dir eq './';
	return File::Spec->catpath('', $dir, $name . "." . $lang . ".translate");
}
...
#sub refreshpot ($) {
# hack ya nadie necesita esta funcion!
#	my $masterfile=shift;
...
#	$doc->{TT}{po_out}->set_charset('UTF-8');
	# do the actual work
#	$doc->parse;
   # hack copiar directamente masterfile a "potfile"
#	IkiWiki::prep_writefile(basename($potfile),dirname($potfile));
#	$doc->writepo($potfile);
#	File::Copy::syscopy($masterfile,$potfile)
#		or error("po(refreshpot) ".
#			sprintf(gettext("failed to create POT file to %s"),
#   			$potfile));
#   
#}
				...
				if (-e "$dir/$pobase") {
					File::Copy::syscopy("$dir/$pobase",$pofile)
						or error("po(refreshpofiles) ".
							 sprintf(gettext("failed to copy underlay TRANSLATE file to %s"),
								 $pofile));
				}
			}
		}
...
#		}
#		else {
		# hack si no existe pofile (.xx.translate) la creamos copiandola de "masterfile"
		if (! -e $pofile) {
			File::Copy::syscopy($masterfile,$pofile)
				or error("po(refreshpofiles) ".
					 sprintf(gettext("failed to copy the TRANSLATE file to %s"),
						 $pofile));
		}
	}
}
...
sub deletetranslations ($) {
	...
	my @todelete;
	map {
		my $file = newpagefile($deletedmasterpage.'.'.$_, 'translate');
...
# hack la funcion en vez de entregar el archivo procesado 
# con los msg de PO entrega directamente el contenido de PO
# que en realidad es el archivo traducido
...
#					"to previous page to continue edit"));
	return IkiWiki::SuccessReason->new("valid data");
}

sub po4a_type ($) {
  # hack esta funcion no es llamada
	my $file = shift;
...
sub match_istranslation ($;@) {
	...
	if (IkiWiki::Plugin::translate::istranslation($page)) {
...
sub match_istranslatable ($;@) {
	...
	if (IkiWiki::Plugin::translate::istranslatable($page)) {
		return IkiWiki::SuccessReason->new("is set as translatable in translate_translatable_pages");
	}
	else {
		return IkiWiki::FailReason->new("is not set as translatable in translate_translatable_pages");
	}
}
...
sub match_lang ($$;@) {
	...
	my $lang=IkiWiki::Plugin::translate::lang($page);
	if ($lang !~ $regexp) {
...
sub match_currentlang ($$;@) {
	...
	my $currentlang=IkiWiki::Plugin::translate::lang($params{location});
	my $lang=IkiWiki::Plugin::translate::lang($page);
...
sub match_needstranslation ($$;@) {
	...
	my $percenttranslated=IkiWiki::Plugin::translate::percenttranslated($page);
